...
export function CalendarMini({ selected, onChange }) {
    ...
    <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setView(new Date(y, m + 1, 1))}
          className="w-8 h-8 rounded-lg bg-white/8 border border-white/10 text-gray-400 hover:text-emerald-400 hover:border-emerald-500/40 transition-colors">›</button>
        <span className="text-sm font-bold text-white">{ARABIC_MONTHS[m]} {y}</span>
        <button onClick={() => setView(new Date(y, m - 1, 1))}
          className="w-8 h-8 rounded-lg bg-white/8 border border-white/10 text-gray-400 hover:text-emerald-400 hover:border-emerald-500/40 transition-colors">‹</button>
      </div>
      <div className="grid grid-cols-7 gap-1 mb-2">
        {["أح","إث","ثل","أر","خم","جم","سب"].map(d => (
          <div key={d} className="text-center text-[10px] text-gray-600">{d}</div>
        ))}
      </div>
      <div className="grid grid-cols-7 gap-1">
        {cells.map((day, i) => {
          ...
          const isSel = selected && selected === `${y}-${String(m+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
          const isToday = date.getTime() === today.getTime();
          return (
            <button key={i} disabled={past}
              onClick={() => onChange(`${y}-${String(m+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`)}
              className={`aspect-square rounded-lg text-xs transition-all
                ...
                ${!isSel && !past && !isToday ? "text-gray-400 hover:bg-white/10" : ""}`}>
              {day}
            </button>
          );
        })}
...
export function SlotPicker({ date, onDateChange, slots, loadingSlots, selectedSlot, onSlotSelect }) {
  const selectedDateObj = date ? new Date(date) : null;
  const dateLabel = selectedDateObj
    ? `${ARABIC_DAYS[selectedDateObj.getDay()]}، ${selectedDateObj.getDate()} ${ARABIC_MONTHS[selectedDateObj.getMonth()]}`
    : "اختر تاريخاً";
      ...
      {date && (
        <div>
          <p className="text-sm text-gray-400 mb-3">
            المواعيد المتاحة — <span className="text-white font-semibold">{dateLabel}</span>
          </p>
          {loadingSlots ? (
